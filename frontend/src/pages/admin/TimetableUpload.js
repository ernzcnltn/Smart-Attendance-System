  ...
  return (
    <Container>
      ...
      <Card className="shadow-sm mb-4" style={{ maxWidth: '600px', margin: '0 auto' }}>
        <Card.Header><strong>{t('admin.timetable.title')}</strong></Card.Header>
        <Card.Body>
          {error && <Alert variant="danger">{error}</Alert>}
          <Alert variant="info" className="small">
            {t('admin.timetable.description')}
            <br />{t('admin.timetable.requiredColumns')} <strong>course_code</strong>, <strong>course_name</strong>, <strong>instructor_email</strong>, <strong>semester</strong>, <strong>attendance_threshold</strong>, <strong>day</strong>, <strong>start_time</strong>, <strong>end_time</strong>
          ...
          <Button variant="outline-secondary" size="sm" className="mb-3 d-flex align-items-center gap-1" onClick={downloadTemplate}>
          ...
          <Button variant="danger" className="w-100" onClick={handleUpload} disabled={!file || loading}>
            {loading ? <Spinner size="sm" /> : t('admin.timetable.title')}
          </Button>
        </Card.Body>
      </Card>
      ...
      {result && (
        <Card className="shadow-sm" style={{ maxWidth: '600px', margin: '0 auto' }}>
          <Card.Header><strong>{t('admin.timetable.resultTitle')}</strong></Card.Header>
          <Card.Body>
            <div className="d-flex gap-3 mb-3 flex-wrap">
              <Badge bg="secondary" className="fs-6">{t('admin.timetable.processed')} {result.processed}</Badge>
              <Badge bg="success" className="fs-6">{t('admin.timetable.schedules')} {result.created}</Badge>
              <Badge bg="primary" className="fs-6">{t('admin.timetable.newCourses')} {result.coursesCreated}</Badge>
              <Badge bg="danger" className="fs-6">{t('admin.timetable.errors')} {result.errors?.length || 0}</Badge>
            </div>
            {result.errors?.length > 0 && (
              <>
                <p className="text-danger small mb-2"><strong>{t('admin.timetable.errors')}</strong></p>
                <Table size="sm" bordered>
                  <tbody>
                    {result.errors.map((e, i) => (
                      <tr key={i}><td className="text-danger small">{e}</td></tr>
                    ))}
                  </tbody>
                </Table>
              </>
            ...
            {result.errors?.length === 0 && (
              <Alert variant="success">{t('admin.timetable.allSuccess')}</Alert>
            )}
          </Card.Body>
        </Card>
      )}
